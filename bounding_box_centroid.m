...
sample = imread('RAW/textline_segmentation.png');                        %must be in parent folder
gray = rgb2gray(sample);
bw = im2bw(gray);
bw = ~bw;                                                   %invert binary image

%bw = imfill(bw, 'holes');                                  %fill holes with pixels
[label num] = bwlabel(bw), title('num');                    %determines the number of bounding boxes to be created
